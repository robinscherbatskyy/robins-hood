...
export default function GlossaryTermLink({ term, compact = false }: { term: GlossaryTerm; compact?: boolean }) {
  return <Link className={`glossary-term-link ${compact ? 'compact' : ''}`} href={`/glossary/${domainSlugForTerm(term)}?q=${encodeURIComponent(term.term)}#${glossaryTermAnchor(term.term)}`}>
    <span>{term.term}</span>
