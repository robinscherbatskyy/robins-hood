...
export default async function CaseDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="case-detail-page">
    ...
    <article className="case-detail-reading">
      ...
      <section className="case-reading-section" id="sources"><header><span>08 · Source record</span><h2>Verify the case at the source.</h2></header><div className="case-source-list">{detail.sources.map((source) => source.url.startsWith('http') ? <a href={source.url} target="_blank" rel="noreferrer" key={source.url}><div><span>{source.publisher}</span><strong>{source.label}</strong>{source.caveat && <small>{source.caveat}</small>}</div><i>↗</i></a> : <Link href={source.url} key={source.url}><div><span>{source.publisher}</span><strong>{source.label}</strong>{source.caveat && <small>{source.caveat}</small>}</div><i>→</i></Link>)}</div></section>
